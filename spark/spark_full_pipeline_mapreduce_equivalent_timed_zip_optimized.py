...
def build_spatial_grid(areas, grid_size=0.01):
    """
    Tworzy prosty indeks przestrzenny dla MODZCTA.
    Kluczem jest komórka siatki lat/lon, a wartością lista poligonów,
    których bounding box przecina daną komórkę.

    Dzięki temu dla jednego punktu NYPD nie sprawdzamy wszystkich ZIP-ów,
    tylko kilka kandydatów z tej samej komórki siatki.
    """
    grid = {}
    polygon_count = 0
    ...
    for zip_code, polygons in areas:
        for ring in polygons:
            ...
            polygon_payload = (zip_code, bbox, ring)
            for i in range(min_i, max_i + 1):
                for j in range(min_j, max_j + 1):
                    grid.setdefault((i, j), []).append(polygon_payload)
...
def create_nypd_zip(nypd_path=NYPD_INPUT, modzcta_path=MODZCTA_INPUT, results_dir=RESULTS_DIR, ts=RUN_TS):
    """
    Optymalizowany etap NYPD_ZIP.

    Najważniejsze zmiany względem pierwszej wersji:
    - najpierw filtrujemy NYPD natywnymi funkcjami Spark, zanim uruchomimy geometrię,
    - poligony MODZCTA są indeksowane w siatce przestrzennej lat/lon,
    - pełny algorytm point-in-polygon uruchamia się tylko dla kandydatów z właściwej komórki,
    - wynik jest persistowany, żeby zapis i liczenie rekordów nie przeliczały drugi raz całej geometrii.
    """
    ...
    grid_bc = spark.sparkContext.broadcast(spatial_grid)

    print(f"MODZCTA: wczytano obszary ZIP={len(areas)}, poligony={polygon_count}, komórki siatki={len(spatial_grid)}")
    ...
    output_schema = T.StructType([
        T.StructField("CMPLNT_NUM", T.StringType(), False),
        T.StructField("ZIPCODE", T.StringType(), False),
        T.StructField("OFNS_DESC", T.StringType(), False),
        T.StructField("BORO_NM", T.StringType(), True),
        T.StructField("LATITUDE", T.DoubleType(), True),
        T.StructField("LONGITUDE", T.DoubleType(), True),
    ])

    def assign_zip_partition(rows):
        grid = grid_bc.value
        ...
        for row in rows:
            ...
            candidates = grid.get(grid_key(lat_value, lon_value, grid_size), [])
            matched_zip = ""
            ...
            if matched_zip:
                yield Row(
                    ...
                    BORO_NM=row["BORO_NM"],
                    LATITUDE=round(lat_value, 6),
                    LONGITUDE=round(lon_value, 6),
                )

    nypd_zip = spark.createDataFrame(
    ...
    versioned_file = f"{results_dir}/nypd_zip_{ts}.csv"
    latest_file = f"{results_dir}/nypd_zip_latest.csv"
    write_single_csv(nypd_zip, versioned_file, latest_file)

    nypd_zip.unpersist()

    # Zwracamy DataFrame odczytany z gotowego pliku latest.
    # Dzięki temu końcowy count w run_timed_stage nie uruchamia ponownie kosztownego spatial join.
